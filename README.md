...
- `DARWIN_OPS_MCP_MODE` default `mock`; set to `local` to enable read-only Linux host collection from mounted `/host/proc`, `/host/etc`, and `/host/usr/lib`
...
- [MCP Approval Architecture](docs/APPROVAL_ARCHITECTURE.md): target layered architecture, tool approval, task approval, and third-party server execution.
